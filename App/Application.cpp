...
static uint32_t                 g_QueueFamily_Compute = (uint32_t)-1;
static VkQueue                  g_Queue = VK_NULL_HANDLE;
static VkQueue                  g_Queue_Compute = VK_NULL_HANDLE;
...
static Celestiq::Application* s_Instance = nullptr;
...
static void SetupVulkan(const char** extensions, uint32_t extensions_count)
{
	...
	// Select graphics queue family
	{
    	...
    	for (uint32_t i = 0; i < count; i++)
    	{
    	    ...
    	    {
    	        g_QueueFamily_Compute = i;
    	        break;
    	    }
    	}
    	// 如果找不到纯计算，就fallback，选任何支持COMPUTE_BIT的
    	if (g_QueueFamily_Compute == (uint32_t)-1)
    	{
    	    for (uint32_t i = 0; i < count; i++)
    	    {
    	...
    	IM_ASSERT(g_QueueFamily_Compute != (uint32_t)-1);
	...
	// Create Logical Device (with graphics + compute queue)
	{
	    ...
	    queue_info[queue_info_count].queueFamilyIndex = g_QueueFamily;
	    queue_info[queue_info_count].queueCount = 1;
	    queue_info[queue_info_count].pQueuePriorities = &queue_priority;
	    queue_info_count++;

	    if (g_QueueFamily_Compute != g_QueueFamily)
	    {
	        queue_info[queue_info_count].sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
	        queue_info[queue_info_count].queueFamilyIndex = g_QueueFamily_Compute;
	        queue_info[queue_info_count].queueCount = 1;
	        queue_info[queue_info_count].pQueuePriorities = &queue_priority;
	        queue_info_count++;
	    }
	    ...
	    if (g_QueueFamily_Compute == g_QueueFamily)
	    {
	        g_Queue_Compute = g_Queue; // 共用
	    }
	    else
	    {
	        vkGetDeviceQueue(g_Device, g_QueueFamily_Compute, 0, &g_Queue_Compute);
	    }
	}
...
static void FramePresent(ImGui_ImplVulkanH_Window* wd)
{
	...
	uint32_t SemaphoreCount = wd->ImageCount+1;
...
namespace Celestiq {
	...
	void Application::Init()
	{
		...
		init_info.RenderPass = wd->RenderPass;  // 额外字段 RenderPass
	...
	const VkInstance Application::GetInstance()
	{
		return g_Instance;
	}

	const VkPhysicalDevice Application::GetPhysicalDevice()
	{
		return g_PhysicalDevice;
	}

	const VkDevice Application::GetDevice()
	{
		return g_Device;
	}

    const VkQueue Application::GetQueue()
    {
		return g_Queue; 
    }

    const VkPhysicalDeviceMemoryProperties &Application::GetPhysicalDeviceMemoryProperties()
    {
		return g_physicalDeviceMemoryProperties.memoryProperties;
    }

    const VkPhysicalDeviceProperties &Application::GetPhysicalDeviceProperties()
    {
        return g_physicalDeviceProperties.properties; 
    }

    const uint32_t Application::GetQueueFamilyIndex()
    {
